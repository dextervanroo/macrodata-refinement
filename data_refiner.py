...
AUTHORIZED_PROCESSORS = ("James.L", "Nora.K", "Arthur.B", "Lena.P", "Felix.G", "Dr.Voss", "Clara.M")
CATEGORIES = ("alpha", "beta", "gamma", "delta")
START_DATE = date(2025, 10, 1)
END_DATE = date(2025, 12, 31)
NORA_END_DATE = date(2025, 11, 15)

def convert_raw_timestamp(value):
    """
    Convert string to a datetime.date value
    """
    try:
...
def read_and_process_mdr(file, raw_data):
    with open(file, "rt") as json_file:
        try:
            ...
            timestamp = convert_raw_timestamp(raw_timestamp)
